...
export async function selectGroup(preset: BasicPreset, options?: Partial<Record<UserName, GroupName | null>>) {
	if (options === undefined) {
		for (const requester of Object.values(preset.requesters)) {
			await requester.profile.selectGroup.mutate({ groupId: preset.groups.Hogwarts.id });
		}
	...
	for (const [_username, groupName] of Object.entries(options)) {
		await preset.requesters[_username as UserName].profile.selectGroup.mutate({ groupId: groupName === null ? null : preset.groups[groupName].id });
	}
}
